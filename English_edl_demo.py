...
def getBasicNER(lang,text):
    ...
    for sentence in doc.sents:
      ...
      endPositions.append(sentIndex)
    
    print('------------------getBasicNER-----------------')
    '''
    print('------------------getBasicNER-----------------')
    print('sentEndPosition',endPositions)
    '''
    res_json={}
    res_json['tokens']=tokens
    res_json['endPositions']=endPositions
...
def getMULTILANG_NER_BERT(lang,text):
    ...
    input = {"lang":'eng',"model":"onto_ner","text":text}
...
def getMULTILANG_NER_COGCOMP(lang,text):
    ...
    #print(res_json)
    #print('----------')
    print('------------------getMULTILANG_NER_COGCOMP-----------------')
    return res_json
'''
...
def processNER(myTabularView,lang,text):
    print('>>>>>>>>>>>>>>>> processNER')
    t,s,annjson = initView(myTabularView, lang, text)
...
def processEDL(myTabularView,lang,text,nerjson={}):
    print('>>>>>>>>>>>>>>>> processEDL')
    annjson = getEnglish_EDL(lang,text)
    #if "tokens" in annjson:
    #    tokens = annjson["tokens"]
    #    if len(tokens) != len(myTabularView.getTokens()): return
    if True and annjson:
...
def doProcess(myTabularView, lang=None, text=None, anns=None):
    ...
    if True or (lang not in ["eng"] and "EDL" in anns):#!!! condition after 'or' seems useless
        edljson = processEDL(myTabularView, lang, text, nerjson)
        print('edl_json', edljson)
        if not edljson:
